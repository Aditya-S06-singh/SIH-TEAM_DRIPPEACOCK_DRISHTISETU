# DrishtiSetu (Attendance & Surveillance Sentinel) — Master System Guide & Codebase Manual (v3.5.1)

> **Executive Summary**: **DrishtiSetu** is an enterprise edge-AI real-time attendance reconciliation, physical security surveillance, and audio intercom system. It solves the critical **Turnstile vs. Physical Occupancy Gap** (ghost attendance, proxy badging, tailgating, and unauthorized departures) by continuously reconciling digital gate/turnstile entries (**Expected Count**) with live computer vision neural tracking (**Detected Count** via YOLO11n). When counts diverge, the system triggers alerts, notifies auditors, streams live video, and enables immediate on-site verification, two-way walkie-talkie communication, and police escalation workflows.

---

## Table of Contents
1. [High-Level Architecture & End-to-End Data Flow](#1-high-level-architecture--end-to-end-data-flow)
2. [What's New in Version 3.5.1](#2-whats-new-in-version-351)
...
6. [Deep Dive: Central Edge AI Vision & Audio Python Engines](#6-deep-dive-central-edge-ai-vision--audio-python-engines)
...
## 2. What's New in Version 3.5.1

- **Lag-Free UI & Video Rendering**:
  - Replaced ad-hoc HTTP connection handshakes with persistent `http.Client()` connection pooling.
  - Decreased polling jitter to a stable 180ms (~5.5 FPS) with in-flight request guards.
  - Implemented downscaled image memory decoding (`cacheWidth: 720`) on Flutter, eliminating Choreographer frame drops.
  - Added dual-gateway fallback (`10.0.2.2` & `127.0.0.1`) so the Android emulator streams seamlessly.
- **Robust Option A AI Engine (`yolo_camera_counter.py`)**:
  - Switched from fragile tracking that crashed on frame transitions (`"WARNING not enough matching points"`) to full-accuracy, resilient **YOLO11 Person Detection (`model.predict(classes=[0], conf=0.35)`)**.
  - Headless-ready `ThreadingHTTPServer` on port `8089` with non-blocking HTTP snapshot ingestion.
  - Automatic reconnection logic when the phone screen turns off and resumes.
- **Two-Way Live Walkie-Talkie Intercom (`live_mic_intercom.py`)**:
  - Continuous streaming from laptop microphone to phone speaker via `sounddevice` (16kHz 16-bit PCM).
  - Background transmission daemon with bounded queue (`maxsize=10`) and stale-packet discard to prevent audio latency accumulation.
  - Native Android `AudioTrack` playback in `MODE_STREAM` on the companion phone.
- **Thermal & Memory Throttling Protection on Phone Node**:
  - Hardware camera configured to `ResolutionPreset.low` with smoothed 1200ms non-blocking frame capture.
  - Eliminates camera hardware shutter locks and keeps the phone cool during continuous operation.
...
│   │   └── zone_model.dart           # Surveillance zone & telemetry state
│   ├── providers/
│   │   └── audit_providers.dart      # Riverpod reactive state repository
│   ├── screens/
│   │   ├── dashboard_screen.dart     # 2x2 Telemetry HUD and zone switcher
│   │   ├── inspection_screen.dart    # Live CCTV viewer, Walkie-Talkie, audit form
│   │   ├── learn_page.dart           # On-device documentation and operational guide
│   │   └── login_screen.dart         # Role-based credentials login
│   ├── services/
│   │   └── appwrite_dashboard_service.dart # Real-time Appwrite telemetry poller
│   └── main.dart                     # Main app entry point
├── drishtisrtuapp-node/              # Companion Phone Camera Node
│   ├── android/app/src/main/kotlin/.../MainActivity.kt # Native AudioTrack PCM player
│   └── lib/
│       ├── screens/sentinel_node_screen.dart   # Camera preview, HUD & capture loop
...
## 4. Deep Dive: The Main Auditor Sentinel App
